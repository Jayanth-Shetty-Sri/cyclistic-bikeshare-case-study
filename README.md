...
- **started_at / ended_at (ride length):** out of ~5.55 million total rides, 147,401 lasted under a minute and 5,585 lasted over a day. A further 29 rows had a negative duration (end time before start time) — a clear data error. The under-a-minute group is almost entirely electric bikes (147,389 of 147,401); the over-a-day group is mostly casual riders on classic bikes (4,677 of 5,585). Both groups are removed from the cleaned dataset as outliers, though the casual-rider pattern is carried forward as a finding rather than discarded entirely.
